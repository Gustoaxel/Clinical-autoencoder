...
def runBestNet(train_dl, test_dl, best_test, outputPath , nfold , class_len , net, feature_name , test_len ):
...
def showMetricsResult(data_train, data_test,  fold_nb):
    ...
    Return:
        ...
        df_acctest: dataframe, testing accuracy per Class in different fold     
    """
    columns = ['Silhouette'] + ['ARI'] + ['AMI'] +['AUC'] + ['Precision'] + ['Recall'] + ['F1 score']
    ind_df = ['Fold '+str(x+1) for x in range(fold_nb)]
...
def Recal_minibatch(X_encoder_pca , Y_encoder_pca , center_mean, data_encoder) : 
    ...
    for i in range(len(Y_encoder_pca)) : 
        ...
        if Y_encoder_pca[i] == 0 and sum(data_encoder[i,:-1] - center_mean[0])<0: 
                X_encoder_pca[i , 0] = data_encoder[i,0] + np.linalg.norm( mean12[0] - mean1[0])
                X_encoder_pca[i , 1] = data_encoder[i,1] + np.linalg.norm( mean12[1] - mean1[1])
                Y_encoder_pca[i] = 3
        if Y_encoder_pca[i] == 1 and sum(data_encoder[i,:-1] - center_mean[1])<0: 
                X_encoder_pca[i , 0] = data_encoder[i,0] + np.linalg.norm( mean21[0] - mean2[0])
                X_encoder_pca[i , 1] = data_encoder[i,1] + np.linalg.norm( mean21[1] - mean2[1])
                Y_encoder_pca[i ] = 4
...
def Data_Recon(X_ajust, Y_ajust, BATCH_SIZE, net, feature_name, doRecal, run_model, patient_name):
    dataset_ajust = LoadDataset(X_ajust,Y_ajust, patient_name)
    X_ajust = torch.utils.data.DataLoader(dataset_ajust, batch_size=BATCH_SIZE,  shuffle=True)
